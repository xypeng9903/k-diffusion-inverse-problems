...
class OpenAIDenoiserV2(DiscreteEpsDDPMDenoiser):
    ...
    def __init__(self, model, diffusion, quantize=False, device='cpu', ortho_tf_type=None):
        alphas_cumprod = torch.tensor(diffusion.alphas_cumprod, device=device, dtype=torch.float32)
        super().__init__(model, alphas_cumprod, quantize=quantize)
        self.out_cov = nn.Conv2d(128, 2 * 3, 1)
    ...
    def forward(self, input, sigma, return_variance=False):
        c_out, c_in = [utils.append_dims(x, input.ndim) for x in self.get_scalings(sigma)]
        model_output, feature = self.inner_model(input * c_in, self.sigma_to_t(sigma), return_feature=True)
        model_output = model_output.chunk(2, dim=1)[0]
        logvar, logvar_ot = self.out_cov(feature).chunk(2, dim=1)
        if return_variance:
